    ...
    <section class="van-section"><div class="container">
        ...
        <div class="vr:grid vr:grid-cols-1 vr:lg:grid-cols-2 vr:gap-8 vr:items-start">
            ...
            <div class="van-panel">
                ...
                <p class="van-small">Valores finais com IVA incluído. Mínimo {{ $van->minimum_hours }} h, faturado por hora iniciada. Antecedência mínima: {{ $van->lead_hours }} h. Levantamento/devolução: {{ substr($van->opens_at, 0, 5) }}–{{ substr($van->closes_at, 0, 5) }}, hora de Lisboa. Duração máxima: 30 dias.</p>
                <div class="vr:my-5">
                ...
                <form method="post" action="{{ route('van-rentals.store', $van) }}" data-reservation-form class="vr:grid vr:gap-5">
                    ...
                    <div class="vr:grid vr:md:grid-cols-2 vr:gap-4">
                        @foreach(['starts_at' => 'Início — Lisboa', 'ends_at' => 'Fim — Lisboa'] as $name => $label)<div class="van-field"><label for="van-{{ $name }}">{{ $label }}</label><input type="datetime-local" step="1800" name="{{ $name }}" id="van-{{ $name }}" value="{{ old($name) }}" required><span class="van-error" data-error="{{ $name }}">@error($name){{ $message }}@enderror</span></div>@endforeach
                    </div>
