<!-- Fleet and How It Works Section -->
    <section class="container pb-5">
      <div class="row g-4">
        <!-- Fleet -->
        <div class="col-lg-6">
          <h3 class="mb-3">A nossa frota</h3>
          <div class="row g-3">
            <div class="col-4">
              <div class="fleet-card">
                <img src="/website/assets/car_sedan.png" alt="Carro Sedan" />
                <p>Carro Sedan</p>
              </div>
            </div>
            <div class="col-4">
              <div class="fleet-card">
                <img src="/website/assets/car_hatch.png" alt="Carro Hatch" />
                <p>Carro Hatch</p>
              </div>
            </div>
            <div class="col-4">
              <div class="fleet-card">
                <img src="/website/assets/car_suv.png" alt="Carro SUV" />
                <p>Carro SUV</p>
              </div>
            </div>
          </div>
        </div>
        <!-- How It Works and FAQ -->
        <div class="col-lg-6">
          <div class="row g-4">
            <div class="col-12">
              <h3 class="mb-3">Como funciona</h3>
              <div class="steps-list">
                <div class="step-item">
                  <div class="step-number">1</div>
                  <div class="step-text">Registe-se na plataforma</div>
                </div>
                <div class="step-item">
                  <div class="step-number">2</div>
                  <div class="step-text">Encontre a viatura ideal</div>
                </div>
                <div class="step-item">
                  <div class="step-number">3</div>
                  <div class="step-text">Comece a conduzir</div>
                </div>
              </div>
            </div>
            <div class="col-12">
              <h3 class="mb-3">Perguntas frequentes</h3>
              <div class="faq-item">
                <h6>Que documentos são necessários para me tornar motorista?</h6>
                <p>
                  Vai precisar do documento de identificação, carta de condução e
                  comprovativo de residência, entre outros.
                </p>
              </div>
              <div class="faq-item">
                <h6>Quais são os requisitos para alugar uma viatura?</h6>
                <p>
                  Ter carta de condução válida e cumprir os critérios de idade
                  mínima previstos pela plataforma.
                </p>
              </div>
              <div class="faq-item">
                <h6>
                  Posso utilizar a minha própria viatura como motorista TVDE?
                </h6>
                <p>
                  Sim, desde que a viatura cumpra os requisitos legais e seja
                  registada na plataforma.
                </p>
              </div>
              <div class="faq-item">
                <h6>Qual é o processo para comprar uma viatura?</h6>
                <p>
                  Contacte-nos para obter informações sobre a nossa oferta de
                  veículos em venda e as condições de aquisição.
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>