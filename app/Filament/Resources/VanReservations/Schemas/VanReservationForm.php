    ...
    public static function configure(Schema $schema): Schema
    {
        ...
        return $schema->components([
            Section::make('Pedido de aluguer')->columns(2)->columnSpanFull()->schema([
                ...
                TextInput::make('period')->label('Período — Lisboa')->formatStateUsing(fn ($record) => $record?->starts_at->timezone('Europe/Lisbon')->format('d/m/Y H:i').' → '.$record?->ends_at->timezone('Europe/Lisbon')->format('d/m/Y H:i'))->disabled()->dehydrated(false)->columnSpanFull(),
