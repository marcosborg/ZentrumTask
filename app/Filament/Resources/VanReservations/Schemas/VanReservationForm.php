    ...
    public static function configure(Schema $schema): Schema
    {
        ...
        return $schema->components([
            Section::make('Pedido de aluguer')->columns(2)->columnSpanFull()->schema([
                ...
                TextInput::make('estimate')->label('Estimativa guardada (IVA incluído)')->formatStateUsing(fn ($record) => $record ? $record->billable_hours.(($record->terms['pricing_unit'] ?? 'hour') === 'two_days' ? ' pacote(s) de 2 dias × ' : ' h × ').number_format($record->hourly_rate / 100, 2, ',', ' ').' € = '.number_format($record->estimated_total / 100, 2, ',', ' ').' €; caução: '.number_format($record->deposit / 100, 2, ',', ' ').' €' : '')->disabled()->dehydrated(false)->columnSpanFull(),
